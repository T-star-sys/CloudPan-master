...
// 向管理服务器发送心跳请求，并打印服务运行时间、请求数和暂禁状态。
bool conCli::heartbeat()
{
    int fd = connectToServer(m_addr,CPT);
    if(fd < 0)
    {
    ...
    stringstream request;
    request<<HEARTBEAT<<" "<<current_user<<" 0\n";
    if(!sendAll(fd,request.str()))
    {
        close(fd);
        cout<<"发送心跳失败"<<endl;
    ...
    if(ret <= 0)
    {
        cout<<"管理服务器没有返回心跳"<<endl;
        return false;
    }

    cout<<string(buf,ret);
    return string(buf,ret).compare(0,2,"OK") == 0;
}

// 交互式发送管理命令；口令应与服务端 CLOUDPAN_ADMIN_TOKEN 环境变量一致。
void conCli::adminCommand()
{
    string token;
    string command;
    cout<<"管理员口令：";
    cin>>token;
    cout<<"命令（STATUS/BAN/UNBAN/RESET/HELP）：";
    cin>>command;

    stringstream request;
    request<<ADMIN<<" "<<token<<" "<<command;
    string upper = command;
    for(size_t i = 0;i < upper.size();++i)
        upper[i] = static_cast<char>(toupper(static_cast<unsigned char>(upper[i])));

    if(upper == "BAN" || upper == "UNBAN")
    {
        string ip;
        cout<<"IPv4 地址：";
        cin>>ip;
