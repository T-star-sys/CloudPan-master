...
namespace
{
const int DEFAULT_TRANSFER_THREADS = 4;

int configuredTransferThreads()
{
    const char *value = getenv("CLOUDPAN_THREADS");
    if(value == NULL || value[0] == '\0')
        return DEFAULT_TRANSFER_THREADS;
...
void FileTrans::Run()
{
    ...
    int threadCount = configuredTransferThreads();
    ...
    if(createPool(threadCount) != 0)
        ERR_EXIT("createPool");
    cout<<"File server thread pool started, workers="<<threadCount<<endl;
    ...
    while(1)
    {
        ...
        for(int i=0;i<events_count;++i)
        {
            ...
            int accepted = accept(trans_fd,(struct sockaddr *)&peer,&peerlen);
            if(accepted < 0)
            {
                if(errno == EINTR)
                    continue;
            ...
            TransferTask *task = static_cast<TransferTask *>(malloc(sizeof(TransferTask)));
            if(task == NULL)
            {
                close(accepted);
                continue;
            ...
            task->conn = accepted;
            if(addWorkToPool(transferTask,task) != 0)
            {
                close(accepted);
                free(task);
            }
...
void *FileTrans::transferTask(void *arg)
{
    TransferTask *task = static_cast<TransferTask *>(arg);
    mysql_thread_init();
    FileTrans session;
    session.connfd = task->conn;
    session.handleConnection();
    mysql_thread_end();
    return NULL;
}

void FileTrans::handleConnection()
{
...
void FileTrans::sendFileInfo()
{
    ...
    if(!db.execSQL(selectSql))
    {
        msg = "ERR mysql query failed\n";
        return;
    }
